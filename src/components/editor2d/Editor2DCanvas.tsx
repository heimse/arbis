...
export function Editor2DCanvas() {
  ...
  const {
    ...
    continueWallDrawing,
    finishWallDrawing,
    startDimensionDrawing,
    updateDimensionDrawing,
    finishDimensionDrawing,
  ...
  const render = React.useCallback(() => {
    ...
    // 7. Рендер размерных линий
    renderDimensions(ctx);

    // 8. Рендер узлов
    renderNodes(ctx);

    // 9. Рендер временной линии при рисовании стены
    if (wallDrawing.active && wallDrawing.startNodeId) {
      renderTempWall(ctx);
    }

    // 10. Рендер временной размерной линии при рисовании
    if (dimensionDrawing.active && dimensionDrawing.startPoint) {
      renderTempDimension(ctx);
    }

    // 11. Рендер snap индикатора
    if (snapPoint) {
    ...
    // 12. Рендер UI элементов (в экранных координатах)
    renderUIOverlay(ctx);
  }, [
    dimensions,
    offset,
  ...
  const renderDimensions = (ctx: CanvasRenderingContext2D) => {
    const layer = plan.layers.find((l) => l.id === 'layer-dimensions');
    if (layer && !layer.visible) return;

    plan.dimensions.forEach((dimension) => {
      const isSelected = selectedType === "dimension" && selectedId === dimension.id;

      ctx.save();
      ctx.strokeStyle = isSelected ? "#4caf50" : "#66bb6a";
      ctx.lineWidth = isSelected ? 2.5 / zoom : 2 / zoom;
      ctx.setLineDash([]);

      const { startPoint, endPoint, offset } = dimension;
      ...
      // Вычисляем точки размерной линии с учетом смещения
      ...
      // Рисуем выносные линии (линии от объектов до размерной линии)
      const extLineLength = 20 / zoom;
      ctx.lineWidth = 1.5 / zoom;
      
      // Выносная линия от начальной точки
      ctx.beginPath();
      ctx.moveTo(startPoint.x, startPoint.y);
      ctx.lineTo(dimStartX, dimStartY);
      ctx.stroke();

      // Выносная линия от конечной точки
      ctx.beginPath();
      ...
      ctx.moveTo(dimStartX, dimStartY);
      ctx.lineTo(
        dimStartX + arrowLength * Math.cos(angle + Math.PI - arrowAngle),
        dimStartY + arrowLength * Math.sin(angle + Math.PI - arrowAngle)
      );
      ctx.moveTo(dimStartX, dimStartY);
      ctx.lineTo(
        dimStartX + arrowLength * Math.cos(angle + Math.PI + arrowAngle),
      ...
      // Стрелка в конце
      ctx.beginPath();
      ctx.moveTo(dimEndX, dimEndY);
      ctx.lineTo(
        dimEndX + arrowLength * Math.cos(angle - arrowAngle),
        dimEndY + arrowLength * Math.sin(angle - arrowAngle)
      );
      ctx.moveTo(dimEndX, dimEndY);
      ctx.lineTo(
        dimEndX + arrowLength * Math.cos(angle + arrowAngle),
      ...
      // Вычисляем и отображаем текст размера
      const lengthPx = length;
      const lengthMm = pixelsToMm(lengthPx, realWorldSize.pixelsPerMeter);
      const text = dimension.text || formatDimension(lengthMm, plan.measurementUnit);
  ...
  const renderTempDimension = (ctx: CanvasRenderingContext2D) => {
    if (!dimensionDrawing.startPoint) return;

    const startPoint = dimensionDrawing.startPoint;
    const endPoint = dimensionDrawing.tempEndPoint || snapPoint || mouseWorldPos;
    ...
    const lengthPx = distance(startPoint, endPoint);
    const lengthMm = pixelsToMm(lengthPx, realWorldSize.pixelsPerMeter);
    const text = formatDimension(lengthMm, plan.measurementUnit);
  ...
  const renderUIOverlay = (ctx: CanvasRenderingContext2D) => {
    ...
    if (tool === "dimension" && dimensionDrawing.active) {
      hints.push("Клик - завершить размерную линию");
    }
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dimensionDrawing.active && dimensionDrawing.startPoint) {
      updateDimensionDrawing(snapPoint || worldPos);
    }

    // Snap
    if ((tool === "wall" || tool === "dimension") && plan.snapSettings.enabled) {
      const gridStep = realWorldSize.pixelsPerMeter / 2;
  ...
  const handleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "select") {
      ...
      const clickedDimension = findDimensionAtPosition(worldPos);
      if (clickedDimension) {
        select(clickedDimension, "dimension");
        return;
      }
    ...
    if (tool === "wall") {
      ...
      if (!dimensionDrawing.active) {
        startDimensionDrawing(usePos);
      } else {
        finishDimensionDrawing(usePos);
      }
    } else if (tool === "room") {
      addRoom(usePos);
