...
export type Editor2DState = {
	...
	updateDimension: (
		id: string,
		partial: Partial<Omit<DimensionLine, "id">>
	) => void;
	...
	updateFurniture: (
		id: string,
		partial: Partial<Omit<FurnitureItem, "id">>
	) => void;
	...
	immer((set, get) => ({
			...
			set((state) => {
				if (state.historyIndex > 0) {
					...
					state.plan = JSON.parse(
						JSON.stringify(state.history[state.historyIndex].plan)
					);
		...
		setPlan: (plan) =>
			set((state) => {
				state.plan = plan;
				state.history = [
					{ plan: JSON.parse(JSON.stringify(plan)), timestamp: Date.now() },
				];
			...
			set((state) => {
				...
				if (state.wallDrawing.startNodeId) {
					const node = state.plan.nodes.find(
						(n: WallNode) => n.id === state.wallDrawing.startNodeId
					);
					if (node && node.connectedWallIds.length === 0) {
						state.plan.nodes = state.plan.nodes.filter(
							(n: WallNode) => n.id !== node.id
						);
					}
				}
		...
		updateWall: (id, partial) =>
			set((state) => {
				const wall = state.plan.walls.find((w: Wall) => w.id === id);
				if (wall) {
		...
		deleteWall: (id) =>
			set((state) => {
				...
				state.plan.doors = state.plan.doors.filter(
					(d: Door) => d.wallId !== id
				);
				state.plan.windows = state.plan.windows.filter(
					(w: Window) => w.wallId !== id
				);
		...
		moveNode: (id, newPosition) =>
			set((state) => {
				const node = state.plan.nodes.find((n: WallNode) => n.id === id);
				if (node) {
		...
		updateDoor: (id, partial) =>
			set((state) => {
				const door = state.plan.doors.find((d: Door) => d.id === id);
				if (door) {
		...
		deleteDoor: (id) =>
			set((state) => {
				state.plan.doors = state.plan.doors.filter((d: Door) => d.id !== id);
				get().pushHistory();
			}),
		...
		updateWindow: (id, partial) =>
			set((state) => {
				const window = state.plan.windows.find((w: Window) => w.id === id);
				if (window) {
		...
		updateDimension: (id, partial) =>
			set((state) => {
				const dimension = state.plan.dimensions.find(
					(d: DimensionLine) => d.id === id
				);
				if (dimension) {
		...
		deleteDimension: (id) =>
			set((state) => {
				state.plan.dimensions = state.plan.dimensions.filter(
					(d) => d.id !== id
				);
				get().pushHistory();
			}),
		...
		moveSelected: (delta) =>
			set((state) => {
				...
				if (selectedType === "node") {
					...
					const furniture = state.plan.furniture.find(
						(f) => f.id === selectedId
					);
					if (furniture) {
		...
		toggleBackgroundVisibility: () =>
			set((state) => {
				if (state.plan.backgroundImage) {
					state.plan.backgroundImage.visible =
						!state.plan.backgroundImage.visible;
				}
			}),
