		...
		case "DELETE_WALL": {
			...
			if (startNode) {
				newNodes.set(wall.startNodeId, {
					...startNode,
					connectedWalls: startNode.connectedWalls.filter(
						(id) => id !== action.id
					),
				});
			}
			...
			if (endNode) {
				newNodes.set(wall.endNodeId, {
					...endNode,
					connectedWalls: endNode.connectedWalls.filter(
						(id) => id !== action.id
					),
				});
			}
...
export function EditorProvider({ children }: { children: React.ReactNode }) {
	...
	const setSelection = useCallback(
		(type: Selection["type"], id: string | null) => {
	...
	const addDoor = useCallback(
		(wallId: string, position: number = 0.5): string => {
	...
	const deleteSelected = useCallback(() => {
		...
		switch (state.selection.type) {
			...
			case "room":
				...
				break;
			case "furniture":
				dispatch({ type: "DELETE_FURNITURE", id: state.selection.id });
				break;
			case "dimension":
	...
	const updateCamera = useCallback(
		(updates: Partial<typeof DEFAULT_CAMERA>) => {
	...
	const zoom = useCallback(
		(delta: number, center?: Point) => {
			const newZoom = Math.max(
				10,
				Math.min(200, state.camera.zoom * (1 + delta))
			);
